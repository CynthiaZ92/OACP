...
/**
  ...
  */
class batchingServer(id: Int, automelt: Boolean)(implicit crdtType: CRDT[Map[String, Set[String]], FollowerEntry]) extends Actor with ActorLogging with FSM[ServerState, Data[Map[String, Set[String]], Map[String, String]]] {
  ...
  var nonUpdate = List.empty[Map[String, String]]
  ...
  when(Follower, stateTimeout = randomElectionTimeout(1500.milliseconds, 3000.milliseconds)) {
    ...
    case Event(msg: BatchingMessage, _) =>
      log.warning("LeaderIKnow: {}", LeaderIKnow)
      sender() ! Resend(LeaderIKnow, BatchingMessage(msg.pending))
...
//      }
    ...
    case Event(msg: BatchingMessage, data) =>
...
//      receiveFlag = false
    ...
    case Event(msg: BatchingMessage, data: Data[Map[String, Set[String]], Map[String, String]]) =>
      log.warning("leader receive nonmonupdate from client inside OACP")
      sender() ! LeaderIs(Some(self))
      if (stateChanged) {
        log.warning("state changed")
